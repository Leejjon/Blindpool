...
export const getCompetitionsThatNeedUpdate = async (): Promise<Result<Array<CompetitionEnum>, ErrorScenarios>> => {
    try {
        ...
        let query = datastore.createQuery('pool')
            ...
            .filter(new PropertyFilter('START_TIMESTAMP', '>', just150MinutesAgo.toJSON())) // But it hasn't gone over 100 minutes.
            .limit(10);
        const result: RunQueryResponse = await datastore.runQuery(query);
        ...
        const competitionKeys = [...new Set(pools.map((pool: Entity) => {
            ...
            const competition = getCompetitionByStringName(match.competitionName);
            console.log(`Pool with id ${pool.ID} found for match ${JSON.stringify(match)} in competition: ${competition}`);
            return competition;
        }))];
